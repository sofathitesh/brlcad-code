	...
	else
	{
		...
		foreach ($review_dir as $name) 
		{
			...
			else
			{
				...
				fclose($file_two);
				fclose($file);
				if(strlen(shell_exec("diff -u -b ".brlcad_source.$real_file_with_path." ".review_queue_directory.$name))=="" or strlen(shell_exec("xmllint  --xinclude --schema ".brlcad_source."resources/other/docbook-schema/xsd/docbook.xsd --noout --nonet ".review_queue_directory.$name." 2>&1"))>100)
				{
